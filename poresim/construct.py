...
class Construct:
    ...
    def _topol_index(self, file_out, path):
        ...
        for mol in self._mols:
            file_out.write(
                f'count{mol}=$(($(grep -c "{mol}" {folder_gro}{file_box})/{self._mols[mol][1]}))\n'
            )
        ...
        if "PORE" in self._struct:
            ...
            file_out.write(cmd)
            file_out.write(
                f'echo "System {self._box_link} - Created pore index file ..."\n'
            )
    ...
    def _pos_dat(self):
        ...
        for mol in self._mols:
            ...
            if (
                is_fill
                and not box_dim
                and section in ("res", "both", "pore")
                and "PORE" in self._struct
            ):
                ...
                if section in ("res", "both"):
                    ...
                    self._write_positions(
                        f"{gro}position_{mol}.dat", np.vstack([top, bot])
                    )
                ...
                for pore_id in self._pore_props:
                    ...
                    if params["central"] != [0, 0, 1] or section not in (
                        "pore",
                        "both",
                    ):
                        continue
                    ...
                    num_pore = int(
                        factor
                        * np.pi
                        * self._pore_props[pore_id]["diameter"] ** 2
                        / 4
                        * params["length"]
                    )
                    positions = np.tile(
                        [centroid[0], centroid[1], res + centroid[2]], (num_pore, 1)
                    )
                    self._write_positions(
                        f"{gro}position_{pore_id}_{mol}.dat", positions
                    )
            ...
            # Pore system: fixed number of molecules
                ...
                if section in ("pore", "both"):
                    for pore_id, j in zip(self._pore_props, range(j)):
                        if pore_id[:5] != "shape":
                            continue
                        if self._pore_props[pore_id]["parameter"]["central"] != [
                            0,
                            0,
                            1,
                        ]:
                        ...
                        res = self._pore_props["system"]["reservoir"]
                        positions = np.tile(
                            [centroid[0], centroid[1], res + centroid[2]], (num[j], 1)
                        )
                        self._write_positions(
                            f"{gro}position_{pore_id}_{mol}.dat", positions
                        )
                ...
                if section in ("res", "both"):
                    ...
                    bot = np.tile(
                        [dims[0] / 2, dims[1] / 2, dims[2] - res / 2], (num[-1], 1)
                    )
                    self._write_positions(
                        f"{gro}position_{mol}.dat", np.vstack([top, bot])
                    )
                    ...
                    for pore_id, j in zip(self._pore_props, range(j)):
                        ...
                        self._write_positions(
                            f"{gro}position_{pore_id}_{mol}.dat", np.vstack([top, bot])
                        )
            ...
            # Box system: density-based fill with area sections
                ...
                for i, seg in enumerate(area):
                    num = int(
                        factor * box_dim[0] * box_dim[1] * (seg[1] - seg[0]) * 0.8
                    )
                    positions = np.tile(
                        [box_dim[0] / 2, box_dim[1] / 2, (seg[1] + seg[0]) / 2],
                        (num, 1),
                    )
            ...
            # Box system: density-based fill without area
                ...
                positions = np.tile(
                    [box_dim[0] / 2, box_dim[1] / 2, box_dim[2] / 2], (num, 1)
                )
            ...
            # Box system: fixed number without area
                ...
                if not box_dim:
                    print(
                        "If you fill a system with one molecule specify in add_mol for every molecule the box dimension"
                    )
                    return
                positions = np.tile(
                    [box_dim[0] / 2, box_dim[1] / 2, box_dim[2] / 2], (inp, 1)
                )
                self._write_positions(f"{gro}position_{mol}.dat", positions)
    ...
    def _structure(self):
        ...
        with open(self._sim_link + "construct.sh", "a", encoding="utf-8") as file_out:
            ...
            if "fill" in [self._mols[mol][0] for mol in self._mols]:
                file_out.write(
                    'echo "Set ions names in sort script if necessary ..."; exit;\n'
                )
            ...
            for mol in self._mols:
                ...
                if "PORE" in self._struct:
                    ...
                    res = self._pore_props["system"]["reservoir"]
                    num = (
                        int(auto_dens / mass / 10 * 6.022 * dims[0] ** 2 * res * 2)
                        if (is_fill and mass)
                        else 0
                    )
                            ...
                            if section != "res":
                                ...
                                file_out.write(
                                    self._gmx_insert_cmd(
                                        ...
                                        nmol,
                                        dr=dr,
                                        pos_file=f"{folder_gro}position_{pore_id}_{mol}.dat",
                                    )
                                )
                        ...
                        for i, seg in enumerate(area):
                            ...
                            file_out.write(
                                self._gmx_insert_cmd(
                                    ...
                                    nmol,
                                    dr=dr,
                                    pos_file=f"{folder_gro}position_{mol}_area{i}.dat",
                                )
                            )
                    else:
                        num = (
            ...
            file_out.write(
                f'echo "System {self._box_link} - Filled simulation box ..."\n\n'
            )
    ...
    def _fill(self):
        ...
        with open(self._box_path + "_fill/fill.sh", "w", encoding="utf-8") as file_out:
            ...
            file_out.write(
                f"cp {folder_fill}$fill_num/{sim_min}/{sim_min}.job {folder_min}\n"
            )
            file_out.write(
                f"cp {folder_fill}$fill_num/{sim_nvt}/{sim_nvt}.job {folder_nvt}\n"
            )
            file_out.write(
                f'echo "System {self._box_link} - Backed up equilibration ..."\n\n'
            )
            ...
            if has_auto_dens:
                ...
                file_out.write(
                    f'echo "System {self._box_link} - Backed up analysis ..."\n\n'
                )
            ...
            for mol in self._mols:
                ...
                if area:
                    for i, seg in enumerate(area):
                        ...
                        dr = [box_dim[0] / 2, box_dim[1] / 2, (seg[1] - seg[0]) / 2]
                        file_out.write(
                            gmx_prefix + f"-dr {dr[0]} {dr[1]} {dr[2]} "
            ...
            file_out.write(
                f'echo "System {self._box_link} - Refilled simulation box ..."\n\n'
            )
            ...
            file_out.write(
                'sed -i "s/fill_num=$fill_num/fill_num=$((fill_num+1))/" temp.sh\n'
            )
            if has_auto_dens:
                file_out.write(
                    'sed -i "s/fill_num=$fill_num/fill_num=$((fill_num+1))/" fillBackup.sh\n'
                )
    ...
    def generate_files(self):
        ...
        for mol in self._struct:
            ...
            if mol == "BOX":
                ...
                utils.copy(
                    file_link, self._box_path + "_gro/" + file_link.split("/")[-1]
                )
