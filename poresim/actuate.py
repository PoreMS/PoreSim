...
class Actuate:
    ...
    def _equilibration(self):
        ...
        * **npt** for pressure equilibration
        ...
        ntomp = {
            step: str(self._job[step]["ntomp"]) if "ntomp" in self._job[step] else "0"
        ...
        for step in self._job:
            if not step == "run":
                ...
                gro_string += (
                    "-n " + folder_gro + file_ndx + " " if self._is_pore else ""
                )
                ...
                gro_string += (
                    str(int(np[step]) * int(nodes[step])) + " "
                    if self._cluster["queuing"]["add_np"]
                    else ""
                )
                gro_string += "gmx_mpi mdrun "
                gro_string += "-s " + sim_all[step] + ".tpr "
                gro_string += (
                    "-ntomp " + ntomp[step] + " " if not ntomp[step] == "0" else ""
                )
        ...
        with open(self._sim_link + "equilibrate.sh", "a") as file_out:
            ...
            file_out.write(
                self._cluster["queuing"]["submit"] + " " + sim_min + ".job\n"
            )
    ...
    def _simulation(self):
        ...
        for i in range(2):
            ...
            utils.replace(
                link_shell, "SIMULATIONGPU", ":gpus=1:exclusive_process" if gpu else ""
            )
            utils.replace(link_shell, "SIMULATIONTIME", wall)
            utils.replace(
                link_shell, "SIMULATIONLABEL", self._label + "_" + str(sim_num)
            )
            utils.replace(
                link_shell, "COMMANDCHANGEDIR", "cd " + self._clr_link + sim_run
            )
            ...
            gro_string += (
                "-maxh " + str(self._job["run"]["maxh"]) + " "
                if "maxh" in self._job["run"]
                else ""
            )
            gro_string += "-dlb no -c -e -g -o -cpo"
        ...
        with open(self._sim_link + "simulate.sh", "a") as file_out:
            ...
            file_out.write(
                self._cluster["queuing"]["submit"] + " " + sim_run + "0.job\n"
            )
            back = "" if self._box_link == "./" else "../"
            file_out.write("cd " + back + "../\n")
            file_out.write("\n")
    ...
    def _analyze(self):
        ...
        utils.replace(
            link_shell, "COMMANDCHANGEDIR", "cd " + self._clr_link + str("ana")
        )
