...
class Simulate:
    ...
    def _update_config(self):
        ...
        sim_dict["box"] = {
            box_id: sim_dict["box"][box_id].get_sim_dict() for box_id in sim_dict["box"]
        }
    ...
    def generate(self):
        ...
        for box_id in self._sim_dict["box"]:
            ...
            box_link = (
                self._link + box.get_name() + "/"
                if len(self._sim_dict["box"]) > 1
                else self._link
            )
            ...
            params = (
                self._sim_dict["param"] if box.get_param() is None else box.get_param()
            )
            ...
            construct = Construct(
                self._link, box_link, box.get_mols(), box.get_struct()
            )
            ...
            for mol in box.get_mols():
                ...
                entry = {
                    "name": mol,
                    "link": mol_link,
                    "target_dens": str(mol_data[2]),
                    "fill": mol_data[0] == "fill",
                }
            ...
            if has_fill:
                Analyze(self._link, box_link, self._sim_dict["cluster"]).extract_mol(
                    "nvt"
                )
            elif not is_pore:
                Analyze(self._link, box_link, self._sim_dict["cluster"]).extract_mol(
